...
export default function TimeSheetPage({ navigation }) {
  ...
  const { addSelection, removeSelection, userSelections } = useSelection();
  ...
  function createPdfHandler() {
    ...
    if (isAdministrator) {
      ...
      userSelections.forEach(function (userSelection) {
        createPDF(date, userSelection.userData.userData, userSelection.userData.workJourneys);
      })
    } else {
      createPDF(date, userData, workJourneys);
    }
  }
  ...
  React.useEffect(() => {
    ...
    const intervalId = setInterval(updateJourneys, 60000);
  ...
  return (
    <View className="flex-1 bg-primary-600">
      ...
      {visible && (
        <MessageModal
          title="Funcionário não selecionado"
          message="Selecione ao menos um funcionário para gerar o relatório."
...
function ExpandableUserJourneyView({ userName, userId, workJourneys, expanded, navigation, addSelection, removeSelection }) {
  ...
  React.useEffect(() => {
    if (isExpanded) {
      addSelection(userId, { userData: users.filter(item => item.id === userId)[0], workJourneys });
    } else {
